...
def alpha_ratio(fft_magnitude, sr):
    ...
    low_band = (freqs >= 50) & (freqs < 1000)
    high_band = (freqs >= 1000) & (freqs < 5000)

    # Calcola l'energia nelle bande di frequenza
    low_band_energy = np.sum(fft_magnitude[low_band, :] ** 2) #np.mean(np.square(fft_magnitude[low_band, :]))
    high_band_energy = np.sum(fft_magnitude[high_band, :] ** 2) #np.mean(np.square(fft_magnitude[high_band, :]))
